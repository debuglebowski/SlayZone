...
interface UndoStackSnapshot {
  canUndo: boolean
  canRedo: boolean
  /** Label of the last undoable action (for tooltip/status) */
  undoLabel: string | undefined
  redoLabel: string | undefined
}

interface UndoStackAPI {
  /** Push a new undoable action onto the stack. Clears redo history. */
  push: (action: UndoableAction) => void
  /** Undo the most recent action. Returns the action label or undefined. */
  undo: () => Promise<string | undefined>
  /** Redo the most recently undone action. Returns the action label or undefined. */
...
export function UndoProvider({ children }: { children: ReactNode }): React.JSX.Element {
  ...
  return <UndoContext.Provider value={stackRef.current}>{children}</UndoContext.Provider>
}
...
export function useUndo() {
  ...
  const push = useCallback(
    (action: UndoableAction) => stack.push(action),
    [stack]
  )
