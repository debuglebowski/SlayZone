...
import { deleteCredential, readCredential, storeCredential } from '../server/credentials'
import type {
  ConnectGithubInput,
  ConnectLinearInput,
...
import { runProviderSync, pushNewTaskToProviders, getDesiredRemoteStatusId, resolveLocalStatus, resolveStatusByCategory } from '../server/sync'
import { providerStatusesToColumns, computeStatusDiff } from '../server/status-sync'
import { htmlToMarkdown, markdownToHtml } from '../server/markdown'
import {
  toMs,
  getProjectColumns,
  normalizeMarkdown,
...
import { getAdapter, getRegisteredProviders, normalizeGithubIssue } from '../server/adapters'
import type { NormalizedIssue, ProviderAdapter } from '../server/adapters'
...
export function createIntegrationOps(db: Database, _options?: { enableTestChannels?: boolean }) {
  ...
  const connectJira = async (input: ConnectJiraInput) => {
    ...
    const { buildJiraCredential } = await import('../server/jira-client')
