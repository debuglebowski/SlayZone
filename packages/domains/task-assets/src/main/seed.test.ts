...
describe('seedInitialVersions', () => {
  ...
  function writeFile(taskId: string, assetId: string, content: string): string {
    const dir = path.join(env.dataDir, 'assets', taskId)
    mkdirSync(dir, { recursive: true })
    const filePath = path.join(dir, `${assetId}.md`)
    writeFileSync(filePath, content, 'utf-8')
    return filePath
  }

  it('seeds v1 for assets without history', () => {
    env.insertAsset('a1', 'task-1', 'note.md')
    env.insertAsset('a2', 'task-1', 'spec.md')
    writeFile('task-1', 'a1', 'first')
    writeFile('task-1', 'a2', 'second')

    const r = seedInitialVersions(env.db, env.txn, env.blobStore, {
      resolveFilePath: (row) => path.join(env.dataDir, 'assets', row.task_id, `${row.id}.md`),
    })
  ...
  it('reports progress', () => {
    env.insertAsset('a1', 'task-1', 'a.md')
