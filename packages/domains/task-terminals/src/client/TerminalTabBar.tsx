...
export const TerminalTabBar = forwardRef<TerminalTabBarHandle, TerminalTabBarProps>(
  function TerminalTabBar(
    ...
    ref: React.Ref<TerminalTabBarHandle>
  ) {
    ...
    const handleDragStart = useCallback((e: React.DragEvent, tab: TerminalTab) => {
      e.dataTransfer.setData(
        DRAG_TYPE,
        JSON.stringify({ tabId: tab.id, sourceGroupId: tab.groupId })
      )
    ...
    const handleNewGroupDrop = useCallback(
      (e: React.DragEvent) => {
        e.preventDefault()
        const raw = e.dataTransfer.getData(DRAG_TYPE)
        if (!raw) return
    ...
    return (
      ...
      >
        ...
        {rightContent && (
          <div className="ml-auto flex items-center shrink-0 pl-2">{rightContent}</div>
        )}
      </div>
    )
  }
