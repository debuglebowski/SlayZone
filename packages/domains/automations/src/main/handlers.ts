...
export function registerAutomationHandlers(ipcMain: IpcMain, db: Database, engine: AutomationEngine): void {
  ...
  ipcMain.handle('db:automations:create', (_, data: CreateAutomationInput) => {
    ...
    db.prepare(
      `INSERT INTO automations (id, project_id, name, description, trigger_config, conditions, actions, sort_order)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      id,
      data.project_id,
  ...
  ipcMain.handle('db:automations:getRuns', (_, automationId: string, limit?: number) => {
    return db.prepare('SELECT * FROM automation_runs WHERE automation_id = ? ORDER BY started_at DESC LIMIT ?').all(automationId, limit ?? 50)
  })
