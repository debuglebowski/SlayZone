...
export const Terminal = forwardRef<TerminalHandle, TerminalProps>(function Terminal({
  ...
  const loadMoreHistory = useCallback(async (): Promise<void> => {
    ...
    try {
      const result = await window.api.pty.getHistoryBefore(
        sessionId,
        archiveStartOffsetRef.current,
        terminalArchiveStepLines,
      )
  ...
  }, [sessionId, terminalArchiveStepLines])
  ...
  const initTerminal = useCallback(async (signal: AbortSignal) => {
    ...
    try {
      ...
      const snapshot = await window.api.pty.getHistorySnapshot(sessionId, terminalArchiveInitialLines)
      if (signal.aborted) return
